...
const defaults: Record<Currency, { avg: number; add: number; cur: number; target: number }> = {
  KRW: { avg: 50_000, add: 40_000, cur: 40_000, target: 45_000 },
  USD: { avg: 100, add: 80, cur: 80, target: 90 },
}
...
export default function AveragePriceCalculator() {
  ...
  const switchCurrency = (c: Currency) => {
    if (c !== currency) {
      setCurrency(c)
      setAvgPrice(defaults[c].avg)
      setAddPrice(defaults[c].add)
      setCurrentPrice(defaults[c].cur)
      setTargetAvg(defaults[c].target)
    }
  }
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,5fr)_minmax(0,7fr)]">
        <section className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-4">
            <Field label="평단가" value={avgPrice} onChange={setAvgPrice} suffix={suffix} step={step} />
            <Field label="보유 수량" value={quantity} onChange={(v) => setQuantity(Math.max(0, Math.round(v)))} suffix="주" step={1} />
            {mode === 'average' ? (
              <>
                <Field label="추가 매수가" value={addPrice} onChange={setAddPrice} suffix={suffix} step={step} />
                <Field label="추가 수량" value={addQty} onChange={(v) => setAddQty(Math.max(0, Math.round(v)))} suffix="주" step={1} />
              </>
            ) : (
              <>
                <Field label="현재가" value={currentPrice} onChange={setCurrentPrice} suffix={suffix} step={step} />
                <Field label="목표 평단가" value={targetAvg} onChange={setTargetAvg} suffix={suffix} step={step} />
              </>
            )}
          </div>
        </section>
            ...
            <>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="divide-y divide-slate-100">
                  ...
                  <Row label="총 투자금액" value={m(avg.totalCost)} />
                  <Row label="새 평단가" value={m(avg.newAvg)} strong />
                </div>
              </div>
            </>
          ) : (
            <>
              {target.possible ? (
                <>
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-6 text-center">
                    <p className="text-sm text-slate-500">
                      평단 {m(targetAvg)}을 만들려면 현재가로
                    </p>
                    <p className="mt-1 text-4xl font-extrabold tabular-nums text-emerald-700">
                      {fmt(target.shares)}주
                    </p>
                    <p className="mt-1 text-sm text-slate-600">
                      약 <b>{m(target.cost)}</b>어치를 추가 매수해야 합니다
                    </p>
                  </div>
                  <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="divide-y divide-slate-100">
                      <Row label="추가 매수 수량" value={`${fmt(target.shares)}주`} strong />
                      <Row label="추가 투자금" value={m(target.cost)} strong />
                      <Row label="매수 후 총 수량" value={`${fmt(quantity + target.shares)}주`} />
                      <Row label="매수 후 총 투자금" value={m(avgPrice * quantity + target.cost)} />
                    </div>
                  </div>
                </>
              ) : (
